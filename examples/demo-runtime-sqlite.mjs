...
console.log("[demo-runtime-sqlite] Starting sqlite-backed runtime recovery demo");

const stateDir = mkdtempSync(join(tmpdir(), "neurocore-runtime-sqlite-"));
const dbPath = join(stateDir, "runtime-state.sqlite");

const agent = defineAgent({
  id: "runtime-sqlite-demo-agent",
  role: "Deterministic high-risk agent used to verify sqlite-backed runtime recovery."
})
  .useReasoner({
    name: "runtime-sqlite-demo-reasoner",
    async plan(ctx) {
      return [
        {
          ...
          explanation: "SQLite runtime recovery path."
        }
      ];
    },
    async respond(ctx) {
        ...
        return [
          {
            ...
            action_type: "respond",
            title: "Return sqlite-backed summary",
      ...
      return [
        {
          ...
          title: "Apply sqlite-backed production cleanup",
...
try {
  ...
  const created = await postJson(`${firstServer.url}/v1/agents/runtime-sqlite-demo-agent/sessions`, {
    tenant_id: "local",
    initial_input: {
  ...
  const approved = await postJson(`${secondServer.url}/v1/approvals/${approvalId}/decision`, {
    approver_id: "sqlite-runtime-demo",
    decision: "approved",
    comment: "Recovered from sqlite store."
  });
