...
**描述**：主 Agent 将子任务派发给子 Agent 执行并汇总结果。这里的子 Agent 不是预先写死的固定三种角色，而是一个由 `capability / domain / tool scope / budget / risk` 描述的开放能力池；主 Agent 在运行时自主决定是否委派、委派几个、委派给哪些角色组合。
...
| FR-PA-04.7 | 主 Agent 自主决定子 Agent 数量、角色和委派层次 | P0 |
| FR-PA-04.8 | 基于 capability / domain / tool scope 的动态子 Agent 选择 | P0 |
| FR-PA-04.9 | 子 Agent 上下文裁剪与最小权限注入 | P1 |

**映射到 Direction A**：完全对应 TaskDelegation (FR-28~FR-35)。

**设计原则**：

- 子 Agent 是运行时角色，不是产品内置的固定流程节点。
- 主 Agent 维护的是根 Goal、约束、预算、审批责任；具体拆成几个 worker、每个 worker 做什么，由主 Agent 在每轮推理中决定。
- 同一类任务在不同上下文下可以编排出不同的子 Agent 组合。例如“做报告”有时只需要搜索 + 综合，有时还会增加 verifier、formatter、邮件投递。
- 子 Agent 可以是长期注册的专长 worker，也可以是围绕单个子 Goal 临时启用的任务角色；两者对主 Agent 的接口保持一致，都是 TaskDelegation。
- 委派给子 Agent 的不是完整主会话，而是裁剪后的 goal、context slice、tool scope 和时间/预算约束。

**子 Agent 组合示例（非穷举，不是固定产品角色）**：

| 任务类型 | 主 Agent 可能编排出的子 Agent 组合 |
|---|---|
| 竞品调研 + 报告 | research-worker、synthesis-worker、verifier、formatter |
| 安排行程 | inbox-reader、calendar-solver、conflict-checker、notifier |
| 邮件处理 | inbox-triage、reply-drafter、approval-guard |
| 项目跟进 | issue-tracker、meeting-summarizer、reminder-worker |
| 私有知识问答 | retrieval-worker、fact-checker、answer-drafter |
| 文件整理 | file-scanner、classifier、rename-worker、archive-worker |
...
NeuroCore Cycle:
    ...
    │
    ├── 本轮动态编排的子 Agent（示例，不是固定集合）
    │   ├── 调研 Worker: 竞品列表 + 特性对比
    │   ├── 综合 Worker: Markdown 报告
    │   ├── 审阅 Worker: 事实和结构检查
    │   └── 格式化 Worker: 转为 PDF
...
Session 1 (max_cycles=50)
    ...
    Cycle 11-30: 子 Agent 动态编排、执行 + 结果收集
    Cycle 31-40: 报告生成
    Cycle 41-48: 等待用户审批
