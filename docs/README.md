...
**M5.1 ~ M9 主线能力已落地，M11 运营控制台进入联调收口阶段。**

- Runtime 主链路、Hosted Runtime、世界模型、设备接入、多 Agent 调度和 Console SPA 脚手架均已进入主分支。
- 当前代码库 `npm run typecheck` 可通过；本地纯逻辑与大多数非 socket 测试可运行。Hosted / `runtime-server` 相关测试仍需在允许本地监听端口的环境中完成全量验证。
- M11 当前不再是“从 0 到 1 开发”，而是“前端 API/WS 契约对齐 + 缺失端点补齐 + 联调验证”。
...
| M8 | 世界模型与设备接入 | ✅ 完成 |
| M9 | 多 Agent 分布式调度 | ✅ 完成 |
| M11 | 运营控制台（Operations Console） | 🔨 开发中（REST/Store/UI 已落地，待契约对齐与联调） |
...
- Console `apiFetch()` 已内置 `/v1` 前缀，但多处页面/Store 仍传入 `/v1/...`，需要统一请求路径约定。
- Console 与 `runtime-server` 的若干响应结构尚未对齐，如 metrics、config、memory、approval/audit 等接口。
- `WsServer` 代码已经落地，但仍需完成服务端启动接线、鉴权约定和前端订阅链路联调。
- Multi-Agent / World Model / Memory / Device 相关页面仍有一批占位视图，依赖补齐 `delegations`、`world-state`、`memory/semantic`、`skills`、`devices` 等端点。

### 下一步建议

- 先统一 Console API 基础层：路径规则、响应 envelope、前端类型定义。
- 再接通 `runtime-server` WebSocket 链路，确认 auth / subscribe / event fan-out 端到端可用。
- 最后补齐 M11 剩余端点并做一次端到端联调，随后再恢复 Console E2E/hosted 测试验证。
