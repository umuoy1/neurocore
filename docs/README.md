...
- 个人助理 Phase A 当前已具备 `IM Gateway / ConversationRouter / Web Chat / Web Search / Web Browser / Agent 组装 / 本地配置覆盖` 等基础能力；审批恢复链路、启动时 `auto_approve` 配置与 proactive 最小闭环也已有定向测试覆盖，但飞书真实卡片链路与可监听端口环境下的完整 Web Chat E2E 仍在进行中。
...
| M12 | 通用自主体能力 | ⬜ 未开始 |
| M13 | 深层元认知与自评估 | ◐ Phase 3 基础闭环已部分进入代码库 |
...
| Prefrontal / Meta | 前额叶 | 96% | 轻量门控、多维评分、冲突检测、prediction error 消费已有；2026-04-14 已落地 Phase 1 基础实现：`MetaSignalBus`、`FastMonitor`、结构化 `MetaAssessment / SelfEvaluationReport`、`WorkspaceSnapshot.metacognitive_state` 与 trace 接线；第二批已补 `DeepEvaluator`、`VerificationTrace`、经 deep eval 校准后的 `MetaAssessment`，以及 `DefaultMetaController` 对 `request-more-evidence / switch-to-safe-response / execute-with-approval / abort` 的安全消费；第三批已补最小 `Calibrator`、`CalibrationRecord` 与 trace/runtime 接线，开始记录执行后置信度校准样本；依据外部子稿一已进一步完成 `FastMonitor V2` 标签化诊断、显式权重、预算抑制，以及 `DeepEvaluator` 按触发标签路由子检查器；依据外部子稿二已补 `MetaSignalBus` 的 `goal_id / provenance / predictor reliability / budget pressure` 聚合规则与字段接线；依据外部子稿三已在 `@neurocore/eval-core` 新增 `meta-benchmark.ts`，覆盖 calibration、fast monitor、deep eval、control allocator、risk gating、evidence sensitivity 与 learning reflection 七组专项指标；`ReflectionLearner`、真实数据集与 online meta eval 仍待后续批次实现 |
| Global Workspace | 全局工作空间 | 80% | broadcast-compete-select 竞争机制已实现 |

### 当前执行顺序

- **当前**：个人助理产品线 + Console 相关准备
  - 个人助理：继续推进 `docs/05_2026-04-01_personal-assistant/` 对应的 Phase A 收口，重点放在飞书接入、审批交互、端到端验证和现有 IM Gateway / Web Chat / 搜索浏览器连接器链路的稳定化。
  - Console：只做与后续 M11 相关的文档整理、接口契约校准、后端支持梳理和必要预实现维护，不以“完成 M11”为当前目标。
- **下一阶段**：`docs/05_2026-04-01_memory-evolution/`
  - 进入五层记忆系统的需求、迁移与验证阶段，作为 Hippocampal 模块深化方向的主线输入。
  - 其中 SQL 记忆库迁移设计、normalized SQLite store、四层记忆 persistence 接线、独立 `SqliteCheckpointStore`、`RuntimeSessionSnapshot` 与 `SessionCheckpoint` 的记忆瘦身、`defineAgent()` 默认 SQL-first 持久化路径、`SqliteRuntimeStateStore` 在 builder/runtime 两条入口的自动补齐链路、legacy SQLite/File runtime state 的显式迁移入口，以及 LongMemEval retrieval benchmark harness、recursive full-bundle loader、matrix/aggregate runner、vendored official retrieval/QA wrapper、hypothesis generation 与 full-run 脚本均已进入代码库；runtime 已不再消费 fat runtime snapshot 的 memory/checkpoint payload，当前剩余重点转向官方全量数据基线跑数、长期兼容性验证和后续 schema 演进。
- **更后阶段**：Prefrontal 深化 + 恢复 M11
  - 在记忆系统演进阶段收口后，推进 `docs/06_2026-04-14_metacognition-evolution/01_deep-metacognition-and-self-evaluation.md`，把当前轻量 Meta 升级为深层自评估系统。
  - M11 则继续按产品化准备节奏推进，待时机合适时恢复正式联调、端到端验证和完整交付。
...
- P0：个人助理 Phase A 收口，优先补齐飞书接入、审批恢复路径、端到端验证，并继续稳定现有 IM Gateway、Web Chat、搜索/浏览器连接器、Agent 组装链路。
- P0：Console 准备项收口，优先统一 API 基础层、WS 接线与前后端字段契约。
- P1：在允许本地端口监听的环境中补跑 hosted / WS / Console 相关回归验证。
- P1：把 multi-agent 的 runtime-server 管理 API、事件编目与可观测性继续收紧为稳定契约。
...
- P2：继续推进 `docs/06_2026-04-14_metacognition-evolution/04_code-first-implementation-tasklist.md` 的后续批次，补 `ReflectionLearner`、更完整的 `MetaControlAction` runtime 消费，以及真实 benchmark 数据集和 online meta eval。
