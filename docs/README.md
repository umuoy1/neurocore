...
- 个人助理 Phase A 当前已具备本地 Web Chat 入口、可直接打开的本地聊天页、基础连接器与 Agent 组装；示例启动配置已支持 `.neurocore/.personal-assistant/` 目录覆盖，并兼容根目录 `.neurocore/llm.local.json` 兜底。
...
- **当前**：个人助理产品线 + Console 相关准备
  - 个人助理：优先推进 `docs/05_2026-04-01_personal-assistant/` 对应的实现计划，先做 IM Gateway、Web Chat、搜索/浏览器连接器、Agent 组装；本地启动配置链路已先打通。
