...
| 3 | [`03_code-audit-checklist.md`](03_2026-03-30_assessment/03_code-audit-checklist.md) | **当前最准确的代码反推审计**——按 M0 ~ M12 逐项判断完成度、闭环状态、架构评估，以及按优先级排序的真实待办 |
...
| 4 | [`04_1_independent-milestones.md`](05_2026-04-01_personal-assistant/04_1_independent-milestones.md) | 独立产品线 milestone 设计——将主线 M0~M12 与个人助理 `PA-M1~PA-M6` 分层，重定义 `M-PA-1~11` 为工程工作包，并给出当前推荐执行顺序 |
...
| Hippocampal / Memory | 海马体 | 98% | 四层记忆 + procedural 自动提炼已有；已修正 procedural 主链路、restore/cleanup 一致性、memory flag、生效中的 retrieval 排序与稀疏向量 rerank、working memory 协议级容量治理、provider/store 边界，以及 procedural/semantic snapshot 持久化恢复与 store reconciliation；SQL 记忆库迁移设计、首批 SQLite store 骨架、四层记忆 persistence 接线、runtime/SDK 显式接线、procedural SQLite skillStore 重载、`RuntimeSessionSnapshot` 与 `SessionCheckpoint` 的四层记忆瘦身、legacy snapshot 一次性 backfill，以及独立 `SqliteCheckpointStore` 均已落地；TTL、negative learning、dense embedding 后端未做 |
...
- **下一阶段**：`docs/05_2026-04-01_memory-evolution/`
  ...
  - 其中 SQL 记忆库迁移设计已启动，首批 schema/store 骨架、四层记忆 persistence 接线、runtime/SDK 显式接线、procedural SQLite skillStore 重载、`RuntimeSessionSnapshot` 与 `SessionCheckpoint` 的四层记忆瘦身、legacy snapshot 一次性 backfill，以及独立 `SqliteCheckpointStore` 均已进入代码库，当前剩余重点转向默认路径切换到 SQL-first restore 与旧 JSON 恢复分支的最终收缩。
- **更后阶段**：恢复 M11
  - 待个人助理和记忆系统演进阶段收口后，再返回 M11，完成 Console 的正式联调、端到端验证和完整交付。
...
- P2：进入 `docs/05_2026-04-01_memory-evolution/`，收敛五层记忆系统迁移方案；当前已完成 SQL 记忆库迁移设计、normalized store 骨架、四层记忆的 SQLite persistence 接线、procedural skillStore 重载、`RuntimeSessionSnapshot` 与 `SessionCheckpoint` 的四层记忆瘦身、legacy snapshot 一次性 backfill，以及独立 `SqliteCheckpointStore`，下一步继续推进默认路径切换到 SQL-first restore 与旧 JSON 恢复分支的收缩。
