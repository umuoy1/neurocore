...
## 11. 个人助理 Agent 竞争规格（2026-04-27）

独立于既有 `05_2026-04-01_personal-assistant/` 产品线文档，对 OpenClaw 与 Hermes Agent 做功能对标，作为下一轮个人助理 agent 的新规格输入。2026-04-27 已按 `PA2-P0-00` ~ `PA2-P2-07` 全链路完成并通过 ledger 验收。

| # | 文档 | 内容 |
|---|---|---|
| 1 | [`01_openclaw-hermes-feature-map.md`](11_2026-04-27_personal-agent-competitive-spec/01_openclaw-hermes-feature-map.md) | OpenClaw / Hermes 对标功能清单——覆盖 runtime/gateway、多渠道、会话、工具、记忆、技能、自动化、多 Agent、MCP、沙箱、治理和 P0/P1/P2 建议 |
| 2 | [`02_architecture.md`](11_2026-04-27_personal-agent-competitive-spec/02_architecture.md) | 个人助理竞争规格架构——PersonalGateway、ConversationRouter、CommandPlane、MemoryFacade、ToolControlPlane、AutomationPlane 与主流程边界 |
| 3 | [`03_delivery-roadmap.md`](11_2026-04-27_personal-agent-competitive-spec/03_delivery-roadmap.md) | 长任务交付路线——Phase 0~7、P0/P1/P2 任务链、阶段退出条件和交付更新格式 |
| 4 | [`04_acceptance-oracle.md`](11_2026-04-27_personal-agent-competitive-spec/04_acceptance-oracle.md) | 验收 oracle——定义 design/unit/integration/E2E/safety/regression/observability 分层验收和产品级场景 |
| 5 | [`05_test-strategy.md`](11_2026-04-27_personal-agent-competitive-spec/05_test-strategy.md) | 测试策略——plan/focused/integration/E2E/safety/benchmark 分层、命令映射、CI lane 和失败处理 |
| 6 | [`06_long-run-agent-protocol.md`](11_2026-04-27_personal-agent-competitive-spec/06_long-run-agent-protocol.md) | 长执行任务协议——开工前检查、执行规则、Ralph loop、完成判定和中断恢复 |
| 7 | [`07_progress-log.md`](11_2026-04-27_personal-agent-competitive-spec/07_progress-log.md) | 长任务进度日志——记录完成项、测试结果、指标和残留风险 |
| 8 | [`08_failed-attempts.md`](11_2026-04-27_personal-agent-competitive-spec/08_failed-attempts.md) | 失败尝试日志——记录失败方案、回滚原因和禁止重复尝试路径 |
| 9 | [`09_personal-assistant-baseline-test.md`](11_2026-04-27_personal-agent-competitive-spec/09_personal-assistant-baseline-test.md) | 个人助理产品级 Baseline 测试规格——完整调用流程、12 轮问答链路、场景矩阵、artifact、指标和回归验收标准 |
| 10 | [`10_gap-requirements-and-execution-plan.md`](11_2026-04-27_personal-agent-competitive-spec/10_gap-requirements-and-execution-plan.md) | PA2 完成后的个人助理缺口需求表与执行计划——30 个 `PA-GAP-*` 需求、Phase A~G 路径和逐项“分析 / 执行 / 验收”过程 |
| 11 | [`project-ledger.json`](11_2026-04-27_personal-agent-competitive-spec/project-ledger.json) | 机器可读任务 ledger——`pa:plan-check / pa:next-task / pa:start / pa:task-check / pa:accept` 的单一真相源 |
...
- OpenClaw/Hermes 对标个人助理任务链已完成：`PA2-P0-00` ~ `PA2-P2-07` 覆盖 gateway、上下文连续性、显式记忆、命令/审批、后台任务、cron、搜索/浏览、多渠道、skills、MCP、subagents、sandbox、standing orders、memory wiki、dreaming、自动技能、profile 隔离、trajectory benchmark、媒体/语音和 Assistant Governance Console。后续是生产化部署联调和规模化验收，不再是该规格任务链的功能缺口。
- 个人助理硅基流动 OpenAI-compatible 链路已完成超时硬化：结构化 JSON 与流式回复超时拆分，`extraBody` 透传，response generation 超时可见兜底，自然语言 precondition 过滤，以及 `complete` 终态直接输出均已进入主链；2026-04-27 真实 WebChat 两轮上下文验证通过。
- 个人助理 PA-GAP 产品化路线已开始执行：`PA-GAP-001` 产品级 Baseline Runner 已落地，当前 deterministic `PA-BL-001` 覆盖 12 轮主链路、S1~S12 场景矩阵和 10 类 artifact，最新 accepted run 为 `pa-bl-001-2026-04-27T15-53-36-881Z-deterministic`，66/66 assertions passed；`PA-GAP-002` 已补齐 `neurocore assistant setup/start/status/stop/install-daemon` 产品入口；`PA-GAP-003` 已补齐 `doctor/health/config --dry-run` 诊断入口；`PA-GAP-004` 已补齐真实交互式 CLI/TUI；`PA-GAP-005` 已补齐 `/retry`、`/undo`、`/personality`、`/insights`、`/trace` 会话 UX 命令，下一项是 `PA-GAP-006` DM pairing / allowlist / home channel。
