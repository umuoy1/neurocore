...
| Hippocampal / Memory | 海马体 | 94% | 四层记忆 + procedural 自动提炼已有；已修正 procedural 主链路、restore/cleanup 一致性、memory flag、生效中的 retrieval 排序与稀疏向量 rerank、working memory 协议级容量治理、provider/store 边界，以及 procedural/semantic snapshot 持久化恢复与 store reconciliation；SQL 记忆库迁移设计与首批 SQLite store 骨架已启动；TTL、negative learning、dense embedding 后端未做 |
...
- **下一阶段**：`docs/05_2026-04-01_memory-evolution/`
  ...
  - 其中 SQL 记忆库迁移设计已启动，首批 schema/store 骨架已进入代码库，当前仍保持 snapshot 持久化作为运行时兼容层。
- **更后阶段**：恢复 M11
...
- P2：进入 `docs/05_2026-04-01_memory-evolution/`，收敛五层记忆系统迁移方案；当前已完成 SQL 记忆库迁移设计，并启动 normalized store 骨架。
