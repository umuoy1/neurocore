...
| 2 | [`02_personal-assistant-architecture.md`](05_2026-04-01_personal-assistant/02_personal-assistant-architecture.md) | 架构设计——3 个新包（im-gateway / proactive-engine / service-connectors）、飞书 Adapter（长连接 + 卡片交互）、IMAdapter SPI、ConversationRouter、Proactive Engine、Service Connector Tool 模式、Agent 组装、飞书技术规格 |
| 3 | [`03_implementation-plan.md`](05_2026-04-01_personal-assistant/03_implementation-plan.md) | 实施计划——Phase A（M-PA-1~4 飞书 + Web Chat + 搜索）、Phase B（M-PA-5~8 主动引擎 + 邮件日历）、Phase C（M-PA-9~11 知识库 + 技能 + 多设备）、依赖关系图、飞书权限要求 |
...
- **当前**：个人助理产品线 + Console 相关准备
  - 个人助理：优先推进 `docs/05_2026-04-01_personal-assistant/` 对应的实现计划，先做 IM Gateway、Web Chat、搜索/浏览器连接器、Agent 组装。
...
- 统一 Console API 基础层：路径规则、响应 envelope、前端类型定义。
- 梳理 `runtime-server` 的 WS 接线、鉴权约定和事件订阅协议，作为未来 M11 联调输入。
- 识别并记录 `delegations`、`world-state`、`memory/semantic`、`skills`、`devices` 等端点缺口，但当前不以“补齐全部页面”为近期目标。

### 真实待办（代码反推）

- P0：个人助理 Phase A 落地，优先做 IM Gateway、Web Chat、搜索/浏览器连接器、Agent 组装。
...
- P2：进入 `docs/05_2026-04-01_memory-evolution/`，收敛五层记忆系统迁移方案。
