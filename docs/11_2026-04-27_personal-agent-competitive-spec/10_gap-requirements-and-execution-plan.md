...
| PA-GAP-005 | 会话 UX 命令 | OpenClaw/Hermes `/retry`、`/undo`、`/personality`、`/insights`、`/trace` | partial | P0 | WebChat/CLI/IM 语义一致，命令不误触模型，trace 可证明 |
...
| PA-GAP-029 | 备份、恢复、同步、加密 | 个人助理长期数据必须可迁移和恢复 | missing | P2 | 一键备份/恢复，跨设备同步冲突可处理，敏感数据加密 |
...
| 3 | 后续从 PA-GAP-003 开始使用 baseline 作为门禁 | 每项任务都有 artifact 和 verdict |
