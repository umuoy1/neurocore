...
> 2026-04-22 状态修正：
> - 本文档保留为设计基线。
> - 当前阶段实现已完成，真实完成状态以 [`../README.md`](../README.md)、[`../03_2026-03-30_assessment/02_gap-analysis-and-roadmap.md`](../03_2026-03-30_assessment/02_gap-analysis-and-roadmap.md) 和 [`milestones/m10-skill-rl.md`](./milestones/m10-skill-rl.md) 为准。
> - 2026-04-22 再修正：FR-47 / FR-48 的当前阶段闭环已补齐，现实现已支持迁移 skill 去重、验证期递减与 penalty 清除、验证失败自动回退，以及基于 TTL 的自动裁剪。
> - 2026-04-23 再修正：FR-45 的当前阶段实现已补齐到“最小上下文化 bandit”。现实现已按 `goal_type / domain / action_type / tool_name / risk_level` 维护 contextual state，并与全局 state 并存；当前仍不是独立神经策略网络。
> - 2026-04-23 再修正：FR-44 / FR-45 当前又补了两处增强。`RewardComputer` 的效率维度已从启发式代理改为基于真实 `cycle / latency / token` 统计；`BanditSkillPolicy` 已从单层 exact-context 升级为 `exact -> operational -> family -> global` 的分层上下文 bandit。
> - 2026-04-23 最终修正：M10 当前阶段已 100% 收口。现实现已补齐 reward `metrics / baseline_metrics` 持久化、基于历史 reward 的相对效率基线，以及 `policy.updated / exploration.triggered / skill.transferred / skill.pruned` 的结构化事件载荷与 SQLite schema 迁移。
...
- [x] `RewardComputer` 可从 Episode 计算出包含 task_completion / efficiency / safety / user_satisfaction 四个维度的 `RewardSignal`
- [x] 每个维度的权重可通过 `AgentProfile` 配置
- [x] 奖励信号支持 `automatic`、`human_feedback`、`prediction_error` 三种来源
- [x] 计算完成后 emit `reward.computed` 事件
- [x] 奖励信号通过 `RewardStore` 持久化，可按 episode_id / skill_id / tenant_id 查询
...
- [x] `SkillPolicy` 接口实现 `selectSkill` 方法，输入候选列表和上下文，输出选择结果及理由
- [x] 策略支持 `update` 方法，接收执行反馈后更新内部参数
- [x] `ProceduralMemoryProvider.retrieve()` 使用 `SkillPolicy` 替代固定 salience 分数
- [x] 策略参数按 tenant 隔离存储
...
- [x] 三种探索策略均可通过 `AgentProfile` 配置切换
- [x] 探索行为受 `risk_level` 约束：`high` 风险技能不参与探索
- [x] 探索率随成功执行次数自动衰减
- [x] 探索决策在 trace 中可查（`SkillSelection.selection_reason = "explore"`）
...
- [x] `SkillEvaluator` 可计算每个技能的综合评分（成功率、平均奖励、使用频率、最后使用时间）
- [x] 评分低于阈值的技能自动标记为 `deprecated`
- [x] 超过 TTL 未使用的技能自动裁剪
- [x] 裁剪前 emit `skill.evaluated` 事件，裁剪后 emit `skill.pruned` 事件
- [x] 裁剪操作可配置为 soft delete（标记）或 hard delete（移除）
...
- [x] `SkillTransferEngine` 可计算两个域之间的相似度
- [x] 迁移时自动调整 trigger_conditions 和 execution_template
- [x] 迁移后的技能初始 confidence 降低，需要经过验证期才恢复
- [x] emit `skill.transferred` 事件，包含 source_domain / target_domain / similarity_score
- [x] 迁移失败的技能自动回退到 fallback_policy
...
- [x] `OnlineLearner` 维护固定大小的经验回放缓冲区
- [x] 支持按 TD-error 优先级采样
- [x] 每 N 个 episode 后自动触发 mini-batch 参数更新
- [x] 更新频率和 batch 大小可通过 `AgentProfile` 配置
- [x] 参数更新不阻塞主认知循环（异步执行）
