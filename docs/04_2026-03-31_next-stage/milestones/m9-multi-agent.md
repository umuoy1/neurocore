...
- [x] 定义 `AgentDescriptor` 接口: agent_id / capabilities / status / heartbeat
- [x] 定义 `AgentRegistry` SPI: register / unregister / discover / heartbeat
...
- [x] 定义 `InterAgentMessage` 接口: sender / recipient / type / payload
- [x] 定义 `InterAgentBus` SPI: send / broadcast / subscribe / unsubscribe
- [x] 实现 `LocalInterAgentBus` (同进程)
- [x] 消息类型: request / response / notification / error
- [x] 消息路由: unicast / multicast / broadcast
- [x] 单元测试: 点对点 / 广播 / 订阅取消 / 错误传播
