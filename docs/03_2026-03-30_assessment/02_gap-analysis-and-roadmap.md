...
> - 2026-04-28 个人助理知识库补充：`PA-GAP-022` 已完成，当前 `SqlitePersonalKnowledgeBaseStore` 支持文档 ingestion、chunk index、citation、delete、reindex、PDF/OCR artifact 和 permission scope；`PersonalKnowledgeBaseRecallProvider` 会把 citation 注入 recall bundle，删除后 search/retrieval 不再返回。
> - 2026-04-28 个人助理联系人图谱补充：`PA-GAP-023` 已完成，当前 `SqliteContactGraphStore` 支持 contact、organization、channel identity 和 relationship；`ContactResolver` 可解析人名、组织、渠道 handle 和关系标签，contact-aware `email_send` 会对歧义收件人要求澄清，并按关系确认策略与 memory_scope 约束发送。
> - 2026-04-28 个人助理语音补充：`PA-GAP-019` 已完成，当前 `VoiceIOService` 支持 audio/voice STT 转写、push-to-talk 标记、assistant TTS 合成、`/voice` 渠道用户偏好和失败文本回退；专项验收覆盖语音输入进入 runtime、语音输出投递和 TTS 失败回退。
> - 2026-04-28 个人助理端侧节点补充：`PA-GAP-020` 已完成，当前 `@neurocore/device-core` 新增 device node gateway protocol，个人助理具备 node pairing code、capability manifest、camera/screen/location/canvas permission gate、headless simulator、设备节点工具和 command audit；专项验收覆盖 runtime 调用 simulator screen command。
