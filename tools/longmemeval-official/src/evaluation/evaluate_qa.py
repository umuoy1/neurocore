...
def get_anscheck_prompt(task, question, answer, response, abstention=False):
    if not abstention:
        if task in ['single-session-user', 'single-session-assistant', 'multi-session', 'single_hop', 'assistant_previnfo', 'two_hop', 'multi_session_synthesis']:
...
Is the model response correct? Answer yes or no only."""
        ...
        elif task in ['temporal-reasoning', 'temp_reasoning_explicit', 'temp_reasoning_implicit']:
...
Is the model response correct? Answer yes or no only."""
            prompt = template.format(question, answer, response)
        elif task in ['knowledge-update', 'knowledge_update']:
            template = """I will give you a question, a correct answer, and a response from a model. Please answer yes if the response contains the correct answer. Otherwise, answer no.
...
Is the model response correct? Answer yes or no only."""
            prompt = template.format(question, answer, response)
        elif task in ['single-session-preference', 'implicit_preference_v2']:
...
Is the model response correct? Answer yes or no only."""
        ...
        template = """I will give you an unanswerable question, an explanation, and a response from a model. Please answer yes if the model correctly identifies the question as unanswerable.
...
if __name__ == '__main__':
    ...
    if metric_model_short in model_zoo:
        ...
        metric_model, metric_model_source = metric_model_short, 'openai'
